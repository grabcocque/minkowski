...
- `Snapshot::save()` / `Snapshot::load()` / `Snapshot::load_zero_copy()` calls
...
- **Zero-copy snapshot loading**: `Snapshot::load_zero_copy()` mmaps the file and copies archived component bytes directly into BlobVec columns — no per-value typed deserialization. Use for large worlds where load speed matters.
...
**Recovery sequence:**
1. Load snapshot: `snap.load(path, &codecs)` or `snap.load_zero_copy(path, &codecs)`
