...
| `planner` | Compiled push-based query planner demo. Cost-based plan compilation, automatic BTree/Hash index selection, join execution, `for_each`/`for_each_raw` iteration, subscription queries with `Changed<T>`, aggregate computation, and `explain()` output. |
