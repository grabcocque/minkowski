...
`QueryPlanner` implements a Volcano/iterator model query planner adapted for in-memory ECS. Plans are compiled to vectorized execution by default — `build()` auto-lowers the logical plan to `VecExecNode` via `lower_to_vectorized()`.

**Logical plan nodes** (`PlanNode`): `Scan` (full archetype iteration), `IndexLookup` (point/range on BTree/Hash), `Filter` (predicate pushdown), `HashJoin` (equi-join on shared component), `NestedLoopJoin` (fallback for small cardinalities).

**Vectorized plan nodes** (`VecExecNode`): `ChunkedScan` (morsel-sized batches per archetype), `IndexGather` (batch index lookup), `SIMDFilter` (branchless filter on contiguous slices), `PartitionedHashJoin` (L2-partitioned build side), `BatchNestedLoopJoin`.
