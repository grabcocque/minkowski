...
- **Transact**: closure-based transaction API. Primary method `transact(&self, world, access, closure)` runs the closure in a retry loop. Impls: `Optimistic` (tick-based validation), `Pessimistic` (cooperative column locks). `Durable<S>` (in persist crate) wraps any `Transact` to add WAL logging.
